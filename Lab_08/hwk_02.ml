(*
I worked with Robert Schonthaler and Michael Zhang, I look about their idea and Micheal recomended me to sort the line befor I compair the lines,
I did what he said but it does not return the right answer, I think the problem that I have when I split the text :( I could not modify it
...
match lst with
|h::tail -> h
|[]-> raise (Invalid_argument "first")

let sort_line line = List.sort (fun x y -> if x < y then -1 else if x = y then 0 else 1) ( List.map (fun x -> List.map (fun z -> List.map Char.lowercase z)  x ) line)

let pairchecking lst n = sort_line(first(take 1 ( drop (n-1) lst))) = sort_line(first(take 1 (drop n lst)))

let unideLine y  = List.map first(List.map (fun x -> if pairchecking y x then [] else (x,x+1)::[]) [1;3;7;9;13;15])
...
let paradelle ( fileName : string ) : result =
	...
	|Some tx -> (let convText = convert_to_non_blank_lines_of_words (tx) in 
		...
		|tx when (length convText != 24) -> IncorrectNumLines (length convText) 
  		|tx when ((sort_line line1 != sort_line line2 )&&(sort_line line11 != sort_line line12 )&& (sort_line line17 = sort_line line18)) ->  IncorrectLines [(1, 2); (11, 12); (17, 18)]
								
	(*	|tx when (line11 != line12 && line17 != line18 ) -> IncorrectLines [(11, 12); (17, 18)] *)
	(*	|tx when (length (unideLine [convText] ) > 0 ) -> IncorrectLines (unideLine [convText])*)
		|tx -> OK )
