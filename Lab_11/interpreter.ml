...
Seq (While (Lt (Var "i", Var "x"),
IfThenElse ( Eq ( Mod( Var "i", Value( Int 2) ) ,Value ( Int 0) ) ,
Seq ( WriteNum (Var "i"), 
Seq (Assign ( "sum_evens", Add (Var "sum_evens", Var "i") ),
         			Assign ("i", Add (Var "i", Value (Int 1)) ) ) ) , 
Seq ( WriteNum (Var "i"), 
Seq (Assign ( "sum_odds", Add (Var "sum_odds", Var "i") ),
         			Assign ("i", Add (Var "i", Value (Int 1)) ) ) ) ) ) , 
...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | While (cond, body) -> 
     (match eval cond stt with
      ...
      | Bool false -> stt
      |  _ -> raise (Failure "incompatible types, While")
     )
  | IfThenElse (e, t, f)-> let ans = eval e stt in 
				(match ans with
				 | Bool true -> exec t stt
				 | Bool false ->  exec f stt
				 | _ -> raise (Failure "incompatible types, IfThenElse")
				) 
  | IfThen ( e, t) -> let ans = eval e stt in
                                (match ans with
                                | Bool true -> exec t stt
				| Bool false -> exec Skip stt 
				| _ -> raise (Failure "incompatible types, IfThen")
				)
  | Skip -> stt
